...
export class ReadOnlyStore implements MemoryStore {
  ...
  /**
   * Reports the inner store's FTS health. A read-only connection cannot run the
   * v1 -> v2 migration, so inspecting a database that has never been opened for
   * writing will honestly report false here rather than repairing it.
   */
  isFtsAvailable(): boolean {
    return this.inner.isFtsAvailable();
  }
